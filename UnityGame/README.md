...
│   │   ├── EnemyChaser.cs        # NavMeshAgent로 플레이어를 실시간 추적, 충돌 시 즉시 게임오버, Animator 있으면 이동 애니메이션 재생
...
│   │   ├── AnimatorUtility.cs    # PlayerController/EnemyChaser가 공유하는 "Animator에 이 float 파라미터가 실제로 있는지" 확인 헬퍼
...
- **걷는 애니메이션이 재생되지 않음**: 캐릭터에 `Animator`와 애니메이션은 있지만, `Player Controller`(플레이어) 또는 `Enemy Chaser`(적)의 **`Speed Parameter Name`**(기본값 `"Speed"`)이 그 캐릭터의 Animator Controller에 있는 실제 파라미터 이름과 다르면 무시됩니다. 캐릭터를 선택해 Inspector에서 `Animator` 컴포넌트의 Controller를 더블클릭해 Animator 창을 열고, `Parameters` 탭에서 이동 속도를 나타내는 float 파라미터 이름을 확인해 `Speed Parameter Name`에 그대로 입력하세요. 참고로 `NavMeshAgent`(적)는 위치·회전만 자동으로 처리할 뿐 Animator를 구동해주지 않으므로, `Enemy Chaser`의 이 연결 코드가 없으면 적은 아예 애니메이션 없이 움직이기만 합니다.
...
- **EnemyChaser (NavMeshAgent)**: `Awake()`에서 `Player` 태그로 플레이어를 찾아두고, `repathInterval`(기본 0.2초)마다 `NavMeshAgent.SetDestination(player.position)`을 호출해 목적지를 갱신합니다. 매 프레임 재계산하지 않고 일정 간격으로만 경로를 다시 잡아 CPU 비용을 줄이는, 실무에서 흔히 쓰는 최적화 패턴입니다. `NavMeshSurface.BuildNavMesh()`로 미리 구워둔 바닥 위를 자율적으로 길찾기하며 이동하고, `OnCollisionEnter`로 플레이어와 부딪히면 **`GameManager.LoseGame()`을 호출해 즉시 게임을 종료**합니다 (리스폰 없음 — 리스폰이 필요한 단순 버전은 `EnemyPatrol`을 쓰세요). `EnemyCharacter.prefab`을 쓰는 경우, `PlayerController`와 마찬가지로 `speedParameterName`(기본 `"Speed"`) 필드로 지정한 Animator float 파라미터에 매 프레임 `NavMeshAgent.velocity.magnitude`를 넣어줘서 이동 애니메이션을 재생합니다 — `NavMeshAgent`는 위치/회전만 제어할 뿐 Animator를 자동으로 구동해주지 않기 때문에, 이 연결 코드가 없으면 캐릭터는 움직이기만 하고 제자리걸음(또는 정지 포즈)처럼 보입니다.
- **AnimatorUtility**: `PlayerController`와 `EnemyChaser` 둘 다 "혹시 이 캐릭터에 지정한 이름의 Animator 파라미터가 실제로 있는지"를 `Awake()`에서 한 번만 확인해야 해서, 그 로직을 공용 정적 헬퍼로 뽑았습니다. 파라미터가 없으면 `_animator` 참조 자체를 `null`로 만들어서, 매 프레임 `SetFloat`을 호출하다 "parameter does not exist" 경고가 반복되는 것을 막습니다.
